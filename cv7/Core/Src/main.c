...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
#define sec 1000 	//waiting one second before print
/* USER CODE END PTD */
...
void StartVisualTask(void const * argument)
{
  /* USER CODE BEGIN StartVisualTask */
	...
	{
		int16_t hodnota;
		if (xQueueReceive(xVisualQueueHandle, &hodnota, portMAX_DELAY)) {
			if (hodnota<-1000){
